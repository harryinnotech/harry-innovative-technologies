    ...
    <article className="group relative flex h-[220px] min-w-0 overflow-hidden rounded-[1.5rem] border border-slate-200 bg-white shadow-sm transition duration-500 hover:-translate-y-1 hover:shadow-2xl sm:h-auto sm:block">
      ...
      <div className="pointer-events-none relative z-10 flex h-full min-w-0 w-full sm:block sm:h-auto">
        ...
        <div className="mt-3 flex min-w-0 flex-row items-center justify-between gap-2 sm:mt-5">
          ...
          <button type="button" disabled={!product.available} onClick={() => addToCart(product)} className="pointer-events-auto max-w-full shrink-0 rounded-lg bg-slate-950 px-2.5 py-2 text-[11px] font-bold leading-tight text-white transition hover:bg-blue-700 disabled:cursor-not-allowed disabled:bg-slate-300 sm:rounded-xl sm:px-4 sm:py-3 sm:text-sm">{product.available ? (product.quoteRequired ? "Request quote" : "Add to cart") : "Unavailable"}</button>
        </div>
        </div>
      </div>
    </article>
