...
function Field({ label, name, value, onChange, error, required = false, type = "text", placeholder, ...props }) {
  return <label className="block text-sm font-bold text-slate-800"><span>{label}{required && <span className="text-red-600"> *</span>}</span><input {...props} name={name} type={type} value={value} onChange={onChange} placeholder={placeholder} aria-invalid={Boolean(error)} aria-describedby={error ? `${name}-error` : undefined} className="mt-2 w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 font-normal outline-none transition focus:border-blue-500 focus:bg-white" />{error && <span id={`${name}-error`} className="mt-1 block text-xs font-semibold text-red-600">{error}</span>}</label>;
}
